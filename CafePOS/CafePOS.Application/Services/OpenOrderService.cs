...
namespace CafePOS.Application.Services
{
    ...
    {
        private IOpenOrderRepository _addItemRepository;
        private int _timeOfDay;

        public OpenOrderService(IOpenOrderRepository addItemRepository, ITimeOfDaySetting timeOfDay)
        {
        ...
        public Result<List<Category>> GetCategories()
        {
            try
            {
                var categories = _addItemRepository.GetCategories(_timeOfDay);
        ...
        public Result<List<ItemPrice>> GetAvailableItems(int categoryId)
        {
            try
            {
                var items = _addItemRepository.GetAvailableItemsByCategory(categoryId, _timeOfDay);
                if (items.Count() == 0)
                {
        ...
        public Result<int> GetItem(int categoryId, int itemId)
        {
            try
            {
                var itemPriceId = _addItemRepository.GetItemPriceID(categoryId, itemId, _timeOfDay);
                if (itemPriceId == 0)
                {
        ...
        public Result AddItemsToOrder(List<ItemToAdd> request)
        {
            try
            {
                _addItemRepository.AddItemsToOrder(request);
        ...
        public Result<List<CafeOrder>> GetOpenOrders()
        {
            try
            {
                var orders = _addItemRepository.GetOpenOrders();
                if (orders.Count == 0)
                {
        ...
        public Result<List<OrderItem>> GetOrderDetails(int orderId)
        {
            try
            {
                var isValid = _addItemRepository.IsValidOrderNumber(orderId);
                var isOpen = _addItemRepository.IsOrderOpen(orderId);
                var details = _addItemRepository.GetOrderDetails(orderId);
